...
export function buildCharacter(input, progress) {
  ...
  node('head', 'torso', [0, 1.39, 0]);
  ring(
    'face',
    'head',
    [
      [-0.16, 0.025, 0.04, 0.015],
      [-0.13, 0.095, 0.086, 0.008],
      [-0.045, 0.157, 0.133],
      [0.06, 0.185, 0.155],
      [0.18, 0.187, 0.158],
      [0.3, 0.16, 0.14],
  ...
  for (const s of [-1, 1]) {
    ...
    ball('iris-' + side, 'head', [s * 0.084, 0.066, 0.158], [0.019, 0.026, 0.006], p.eyeColor);
    ball('pupil-' + side, 'head', [s * 0.082, 0.066, 0.164], [0.007, 0.017, 0.003], outline);
    ball('eye-glint-' + side, 'head', [s * 0.076, 0.077, 0.167], [0.004, 0.005, 0.002], '#ffffff');
    stroke(
      'eye-top-' + side,
      'head',
      [
    ...
    const shoulderX = 0.41 + (m + gain('Shoulders')) * 0.07;
    ...
    ring(
      ...
      [
        [0, 0.095, 0.101],
        [-0.12, 0.13 + (m + gain('Arms')) * 0.027, 0.13],
        [-0.32, 0.118 + m * 0.018, 0.121],
        [-0.48, 0.077, 0.079],
        [-0.51, 0.072, 0.073],
      ],
      skin,
...
function addHair(p, node, add, ball) {
  ...
  const cap = surface(
    [
      [0.085, 0.179, 0.14, -0.012],
      [0.21, 0.199, 0.167, -0.012],
      [0.32, 0.164, 0.144, -0.005],
      [0.38, 0.065, 0.07],
      [0.385, 0.001, 0.001],
    ],
    32,
